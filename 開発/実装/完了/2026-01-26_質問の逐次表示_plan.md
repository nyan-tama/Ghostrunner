...
| ファイル | 変更内容 |
|---------|---------|
| `frontend/src/app/page.tsx` | `currentQuestionIndex` 状態追加、`setQuestionsWithReset` 関数追加、`handleAnswerWithNext` 関数追加 |
| `frontend/src/components/ProgressContainer.tsx` | props に `currentQuestionIndex` 追加、`QuestionSection` に `key` と `currentQuestionIndex` を渡す |
| `frontend/src/components/QuestionSection.tsx` | props に `currentQuestionIndex` 追加、状態を簡略化、進捗表示追加、単一質問表示に変更 |
| `frontend/docs/screens.md` | 質問の逐次表示セクション追加、データフロー図更新 |
| `frontend/docs/screen-flow.md` | 質問回答フローの詳細追加、全体フロー図更新 |

### 技術的アプローチと設計判断

#### 状態管理の設計

- `currentQuestionIndex` を親コンポーネント（`page.tsx`）で管理
- `questions` 配列の更新時に `setQuestionsWithReset` で同時にインデックスをリセット
- `QuestionSection` は現在の質問のみを表示（`questions[currentQuestionIndex]`）

#### コンポーネントのリマウント戦略

- `ProgressContainer` で `QuestionSection` に `key={currentQuestionIndex}` を付与
- 質問が切り替わるとコンポーネントがリマウントされ、入力状態（`customAnswer`, `selectedOptions`）が自動的にリセット
- これにより、明示的な状態リセット処理が不要になり、コードがシンプルに
...
- `ProgressContainer` の `QuestionSection` に `key={currentQuestionIndex}` を追加し、質問が変わるとコンポーネントがリマウントされるようにした（入力状態の自動リセット）
- `useEffect` による `questions` 監視ではなく、`setQuestionsWithReset` 関数で質問セットと同時にインデックスをリセットする方式を採用（より直接的で確実）
...
| ESLint | Pass | 警告なし |
| ビルド | Pass | 既存の警告のみ（本変更に起因するものはなし） |
...
1. **アニメーション追加**: 質問切り替え時にフェードやスライドアニメーションを追加すると、UXが向上する可能性がある
2. **キーボード操作**: 数字キーで選択肢を選べるようにすると、パワーユーザーの操作性が向上
3. **回答の戻り機能**: 前の質問に戻って回答を変更できる機能（現在は前に戻れない）
...
- 「質問 1/3」のような進捗表示が追加
- 最後の質問以外への回答時は、即座に次の質問が表示（待ち時間なし）
