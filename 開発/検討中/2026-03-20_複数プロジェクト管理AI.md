...
| 項目 | 決定 |
|------|------|
| 対象 | 全プロジェクトではなく、**指定プロジェクトのみ** |
| 並列数 | **5個まで並列**実行、超過分は順次キュー待ち |
| トリガー | 3種類全て対応: 手動ボタン + 定期ポーリング + plan検知 |
| 承認（ツール許可） | **自動承認**（bypassPermissions） |
| 承認（設計判断） | **通知して待つ** → ntfy でスマホ通知 → 人が回答 |
| 承認待ち中の挙動 | そのプロジェクトは停止、**他のプロジェクトは進行**を続ける |

### トリガーの実装方針

- **手動ボタン**: ダッシュボードに「巡回開始」ボタン。MVPで最初に作る
- **定期ポーリング**: 5分間隔で指定プロジェクトをスキャン
- **plan検知**: ポーリングの中で実現（`開発/実装/実装待ち/` にファイルがあれば未処理タスクあり）

定期ポーリングとplan検知は同じ実装で実現できる。

### 承認の使い分け

| 場面 | 扱い |
|------|------|
| ファイル編集・コマンド実行の許可 | 自動承認（bypassPermissions） |
| 実装方針の判断（「AとBどちらにしますか？」等） | ntfy通知 → 人が回答 → --resume で再開 |

### 巡回フロー

```mermaid
flowchart TD
    START[巡回開始] --> SCAN[指定プロジェクトをスキャン]
    SCAN --> CHECK{開発/実装/実装待ち/ にファイルあり?}

    CHECK -->|なし| SKIP[スキップ]
    CHECK -->|あり| SLOTS{並列スロット空き?}

    SLOTS -->|空き| EXEC["claude -p /coding 実行<br/>(bypassPermissions)"]
    SLOTS -->|5個実行中| QUEUE[キューに追加]

    EXEC --> RUNNING{実行中}
    RUNNING -->|設計判断の質問| NOTIFY[ntfy通知 → 停止して待機]
    RUNNING -->|完了| DONE[完了 → スロット解放]
    RUNNING -->|エラー| ERR[エラー通知]

    DONE --> NEXT[キューから次を取り出し]
    NOTIFY --> RESUME[人が回答 → --resume で再開]
    RESUME --> RUNNING
```

## MVP提案

**推奨案: 案C（ハイブリッド）を上記の確定事項で実装**

### MVPスコープ（フェーズ1）

**目標: 「指定プロジェクトの巡回を手動ボタンで開始し、自動で作業を進める」**

必須機能:
1. **プロジェクト登録**: 巡回対象プロジェクトの指定（パス指定）
2. **状態スキャン**: 各プロジェクトの `git log --oneline -5` と `開発/実装/実装待ち/` の状態を収集
3. **ダッシュボード画面**: 指定プロジェクトの状態一覧 + 「巡回開始」ボタン
4. **巡回実行エンジン**: 5並列スロット + キュー管理、claude -p で /coding 実行
5. **承認フロー**: 設計判断の質問を検出 → ntfy通知 → ダッシュボードで回答 → --resume
6. **進捗表示**: 各プロジェクトの実行状態（待機/実行中/承認待ち/完了/エラー）をリアルタイム表示

### フェーズ2以降（次回以降）

- **定期ポーリング**: 5分間隔の自動スキャン + 自動実行
- **優先順位レコメンド**: スキャン結果をclaude -pに渡し、今日やるべきタスクを提案
- **実行履歴**: 各プロジェクトの実行ログ・コスト履歴を蓄積
- **Agent Teams統合**: 安定したらTeammateによる並列実行を検討
...
1. `/plan` でMVPの実装計画を作成
2. devtoolsのバックエンド（Go）に巡回実行エンジン + スキャンAPIを追加
3. devtoolsのフロントエンド（Next.js）にダッシュボード + 承認UIを追加
