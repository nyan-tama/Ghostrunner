...
flowchart TD
    subgraph fullstack["/fullstack"]
        FS1["実装計画<br/>(DB変更の有無を判断)"]
        FS2["DB設計 (pg-planner)<br/>DB実装 (pg-impl)<br/>(あれば)"]
        FS3["テストDB検証 (pg-tester)<br/>(あれば)"]
        FS4["アプリ実装<br/>(front/back必要な方だけ)"]
        FS5["テスト"]
        FS6["レビュー<br/>(DB変更ありならpg-reviewerも)"]
        FS7["コミット → featブランチ"]
...
実装からfeatブランチへのコミットまでを担当する。staging操作は行わない。

```mermaid
flowchart TD
    START["/fullstack"] --> PLAN["1. 実装計画<br/>(DB変更の有無を判断)"]
    PLAN --> DB_CHECK{"DB変更あり？"}
    DB_CHECK -->|あり| DB_PLAN["2. DB設計 (pg-planner)"]
    DB_CHECK -->|なし| APP_IMPL

    DB_PLAN --> DB_IMPL["3. DB実装 (pg-impl)"]
    DB_IMPL --> DB_TEST["4. テストDB検証 (pg-tester)"]
    DB_TEST --> APP_IMPL["5. アプリ実装<br/>(front/backも必要な方だけ)"]

    APP_IMPL --> TEST["6. テスト"]
    TEST --> REVIEW["7. レビュー<br/>(DB変更ありならpg-reviewerも)"]
    REVIEW --> COMMIT["8. コミット → featブランチ"]
...
| フロントだけ | バックエンド実装 |
| バックだけ | フロントエンド実装 |
| DB変更なし | DB設計、DB実装、テストDB検証、pg-reviewer |
