...
```
ローカル git → push → GitHub → Cloud Build トリガー → Cloud Run デプロイ
```

| トリガー | デプロイ先 | 起動タイミング |
|---------|-----------|--------------|
| `staging` ブランチに push | staging Cloud Run | `/stage` 実行時 |
| `main` ブランチに push | 本番 Cloud Run | `/release` 実行時 |

**エージェントの責務:**
- staging-manager: git操作（squash merge + push）のみ。デプロイはCloud Buildが自動実行
- release-manager: git操作（merge + push）のみ。デプロイはCloud Buildが自動実行
- エージェントがデプロイコマンドを知る必要がない
