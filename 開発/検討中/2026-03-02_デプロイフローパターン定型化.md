...
| 2026-03-02 | 方針追加: インフラ構成を確定（Cloud Run + Supabase + Secret Manager、Cloud Buildトリガーデプロイ） |
| 2026-03-02 | 方針追加: DBマイグレーションはCloud Build内で実行（staging/本番共通） |
...
| 環境 | アプリ | DB | シークレット管理 |
|------|--------|-----|----------------|
| 開発（ローカル） | Makefile (`make dev`) | Docker (PostgreSQL) | `.env` ファイル |
| staging | Cloud Run (staging) | Supabase (別プロジェクト) | Secret Manager |
| 本番 | Cloud Run (prod) | Supabase (本番) | Secret Manager |

### ホスティング: Cloud Run

- Go + Cloud Run は相性が良い（コールドスタート約200ms）
- 個人開発なら `min-instances=0`（デフォルト）で無料枠内に収まる
- 気になれば `min-instances=1` でコールドスタート解消（待機コストはメモリ分のみ）

### DB: Supabase (PostgreSQL)

- GORMは素のPostgreSQLとして接続するため、開発Dockerと本番Supabaseの差異はほぼ出ない
- Supabase無料枠2プロジェクト = staging + 本番でちょうど収まる
- Supabase固有機能（RLS, Auth, Realtime等）に依存する場合は、開発もSupabase CLI（ローカル）に切り替え可能

### シークレット管理: Secret Manager

deploy.shでの環境変数管理をやめ、Google Secret Managerに移行する。

**使い分け:**

| 種類 | 置き場所 |
|------|---------|
| DB接続文字列、APIキー、OAuthシークレット | **Secret Manager** |
| 環境名(dev/staging/prod)、ポート番号、ログレベル | 環境変数でOK |
...
```
コード側は `os.Getenv("DATABASE_URL")` で読むだけ。Secret Managerを意識する必要なし。

**メリット:**
- deploy.shからシークレット管理が不要になる
- staging/本番で接続先の切り替えが容易（各環境のSecretを参照するだけ）
- アクセス監査ログ付き
- 月10,000アクセスまで無料（個人開発なら余裕）

### デプロイ方法: Cloud Build トリガー

**GitHubブランチへのpushをトリガーに自動デプロイする。**

```
ローカル git → push → GitHub → Cloud Build トリガー → マイグレーション → Cloud Run デプロイ
```

| トリガー | デプロイ先 | 起動タイミング |
|---------|-----------|--------------|
| `staging` ブランチに push | staging Cloud Run + staging Supabase | `/stage` 実行時 |
| `main` ブランチに push | 本番 Cloud Run + 本番 Supabase | `/release` 実行時 |

**Cloud Build実行ステップ:**
```yaml
# cloudbuild.yaml
steps:
  # 1. DBマイグレーション実行（デプロイ前に1回だけ）
  - name: 'golang'
    entrypoint: 'go'
    args: ['run', './cmd/migrate']
    secretEnv: ['DATABASE_URL']

  # 2. アプリをCloud Runにデプロイ
  - name: 'gcr.io/cloud-builders/gcloud'
    args: ['run', 'deploy', '...']
```

**マイグレーション実行方式の選定理由:**
- Cloud Build内で実行（アプリ起動時のAutoMigrateではない）
- デプロイごとに1回だけ実行（スケールアウト時の同時実行問題なし）
- マイグレーション失敗時はデプロイが中断される（安全）
- PgBouncer経由ではなく直接接続でDDLを実行
- staging/本番で同じcloudbuild.yamlを使用（接続先DBのみトリガーで切替）

**エージェントの責務:**
- staging-manager: git操作（squash merge + push）のみ。マイグレーション+デプロイはCloud Buildが自動実行
- release-manager: git操作（merge + push）のみ。マイグレーション+デプロイはCloud Buildが自動実行
- エージェントがデプロイコマンドやマイグレーションコマンドを知る必要がない

### ローカル開発: Makefile

```bash
make dev              # Go + Next.js + Docker PG を全部起動
make backend          # Go単体
make frontend         # Next.js単体
make db               # Docker PG起動
make stop             # 全停止
```
...
各プロジェクトのCLAUDE.mdに以下を記載する（グローバルには書かない。プロジェクトごとに構成が異なるため）。
...
deploy: cloud-build (branch trigger)

## 環境
dev: local (makefile + docker)
staging: cloud-run + supabase (myapp-staging)
production: cloud-run + supabase (myapp-prod)
```
