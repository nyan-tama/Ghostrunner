...
作成日: 2026-05-25
ステータス: 設計検討中（各論点に案＋推奨を提示。最終決定はメインスレッド。MVP提案まで到達、次は /plan）
...
1. 本検討書の運用 Phase1（把握先行）を `/plan` で実装計画化する。
   - 主要成果物: (a) auto-follow userscript の進捗 POST（meta.js の grant/connect 追加＋送信関数）、
     (b) x-liker backend の受信エンドポイント＋ステータスファイル書き出し（registry パターン）、
     (c) x-liker の `運用/manifest.json`、(d) chief-director への運用軸追加、(e) CLAUDE.md「統括（運用把握）」節。
2. 計画確定後、計画書を `開発/実装/実装待ち/` に移動。
3. 運用 Phase1 完了後、運用 Phase2（bulk-ops 一括発火）を別計画で設計・実装。
4. （後フェーズ）点火後の結果通知・stale 自動解消（人間トリガー）・devtools 運用可視化を検討。
