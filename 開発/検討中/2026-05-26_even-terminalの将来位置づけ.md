...
#### Q2-a への回答（2026-05-26）

| 論点 | ユーザー回答 |
|------|-------------|
| 確認事項回答（A 案 / B 案）を Even G2 から音声でやるか | **◎ やりたい（音声化する）** |
| TTS の二重実装問題（dashboard ⇄ Even G2 排他） | **運用で十分（コードロック不要）** |

確定した含意:

- **Even G2 からの確認事項音声回答を実装範囲に含める**。
   - 「A 案で」「B 案で」等の音声入力で `**ステータス**: 回答済` への書き戻しを発火させる。
   - 書き戻し API は dashboard MVP で実装済（POST /api/answer 相当）→ **同 API を Even G2 経由でも叩く**設計。サーバ側に新規エンドポイントは不要、Even G2 側（even-terminal の音声インテント解釈）から既存 API を呼ぶブリッジ層を整備する。
   - 場面マトリクス的に「移動中・歩行 × 確認事項に即返答したい」シーンを正式にサポート。
   - 一括操作系（bulk-coding）と違って影響範囲が単一プロジェクト・1 行に限定されるため、誤発火リスクは許容範囲。明示動詞ゲート（「A 案で」「B 案で」「回答 A」等）で十分。
- **TTS の排他制御はコードレベルで実装しない**。
   - dashboard 側に既に TTS ON/OFF トグル実装済（MVP 完了済機能）→ Even G2 利用中はユーザーが手動 OFF にする運用で足りる。
   - 場面マトリクス的に **dashboard と Even G2 を同時使用するシーンは稀**（一覧性が要るなら dashboard、ハンズフリーなら Even G2、と用途が直交）。
   - 自動排他ロジック（dashboard が Even G2 の状態を検知して TTS 抑制 / その逆）を実装するコストに対して、得られる UX 改善が小さいとの判断。
   - 万一頻繁に衝突するようになれば、後追いで「Even G2 active 中フラグ」を even-terminal 経由で dashboard に通知して自動 mute する拡張は可能（後フェーズ）。
- **`/plan` フェーズに含めるタスク**:
   - (a) Even G2 ⇄ dashboard 確認事項回答 API の共通化確認（dashboard MVP の書き戻し API が provider 中立に呼べるか）
   - (b) Even G2 側の音声インテント設計（「A 案で」「B 案で」等の発話パターンと回答書き戻し API のマッピング）
   - (c) 回答対象 session の解決ロジック（直近確認事項を持つ session / 計画書を特定するヒューリスティック）

#### Q2-a 解決後の本書ステータス

- **本書の全論点が解決**（Q1 / Q2 / Q2-a / Q3 / Q3-a / Q4 すべて回答済）。
- 残るは `/plan` 直前の **実機検証タスク (i)〜(iii)** のみ（案 ζ の前提潰し）。これは検討事項ではなく実機作業なので別タスク。
- 検討フェーズとしては **クローズ可能**。`/plan` への移行準備が整った状態。
...
- [x] ~~Even G2 経由での統括スキル音声発火~~ → 把握系 ◎ / 一括操作系 ×（Q2 解決）／確認事項回答音声化 ◎・TTS 排他は運用で十分（**Q2-a 解決**）
...
- [x] ~~**新規 Q2-a**: 確認事項回答の音声化 / TTS 排他制御~~ → **全解決（2026-05-26）。音声化は実装する（既存書き戻し API を Even G2 経由でも呼ぶ）／TTS 排他は運用で十分（dashboard 側既存トグルで足りる）**

**→ 本書の全論点が解決。検討フェーズはクローズ可能。次は `/plan` 直前の実機検証 (i)〜(iii) を別タスクで実施 → `/plan` へ移行。**

## 次のステップ

1. ~~**Q4-0a / Q4-0b / Q4-0c の確認事項回収**~~ → **全解決（2026-05-26）**:
   - Q4-0a = ホスト機と同一 / Q4-0b = VSCode の Claude Code プラグイン / Q4-0c = 最初から共有実装（案 ζ）
2. ~~**残論点 Q2-a の回答取得**~~ → **全解決（2026-05-26）**: 音声化は実装する／TTS 排他は運用で十分
...
5. 実機検証 (i)〜(iii) 完了後、`/plan` で **複数系統の実装計画**を作る:
   ...
   - (d) **新規実装タスク（Q2-a 部分・Even G2 確認事項音声回答）**: Even G2 側の音声インテント（「A 案で」「B 案で」等）から dashboard MVP 既存の回答書き戻し API を呼ぶブリッジ整備。サーバ側 API は流用（新規エンドポイント不要）、回答対象 session 解決ヒューリスティック設計。TTS 排他は実装しない（運用で対応）。
   - ~~(e) iTerm2 用 CLI ラッパー対（案 ε）~~ → **Q4-0b で VSCode 確定により降格、当面実施不要**
   - ~~(f) claude バイナリ TUI 直接アタッチ（案 α）~~ → **Q4-2 で粒度 4 不要が確定したため降格**
