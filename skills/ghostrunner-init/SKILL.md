...
- 選択肢:
  1. データベース（ユーザー情報、注文履歴など、表形式のデータを保存したい場合）
  2. ファイルストレージ（画像、PDF、動画などのファイルをアップロード・管理したい場合）
  3. キャッシュ（よく使うデータを高速に取り出したい場合。分からなければ選ばなくてOK）
  4. 特になし（まずはシンプルに始めたい）
...
作成するプロジェクト:
  名前: <名前>
  作るもの: <Q1の回答>
  データの保存: データベース, ファイルストレージ（選択したもの）
  生成先: /Users/user/<名前>/
...
cp -r ${CLAUDE_PLUGIN_ROOT}/templates/with-db/backend/internal/. /Users/user/<プロジェクト名>/backend/internal/

# DB 初期化 SQL + docker-compose.yml を追加
cp -r ${CLAUDE_PLUGIN_ROOT}/templates/with-db/db/. /Users/user/<プロジェクト名>/db/
cp ${CLAUDE_PLUGIN_ROOT}/templates/with-db/docker-compose.yml /Users/user/<プロジェクト名>/docker-compose.yml

# フロントエンド /samples ページを追加
mkdir -p /Users/user/<プロジェクト名>/frontend/src/app/samples
cp ${CLAUDE_PLUGIN_ROOT}/templates/with-db/frontend/src/app/samples/page.tsx /Users/user/<プロジェクト名>/frontend/src/app/samples/page.tsx
```

ストレージ選択時:
```bash
# Go ソース（registry, handler, infrastructure）を追加
cp -r ${CLAUDE_PLUGIN_ROOT}/templates/with-storage/backend/internal/. /Users/user/<プロジェクト名>/backend/internal/

# フロントエンド /storage ページを追加
mkdir -p /Users/user/<プロジェクト名>/frontend/src/app/storage
cp ${CLAUDE_PLUGIN_ROOT}/templates/with-storage/frontend/src/app/storage/page.tsx /Users/user/<プロジェクト名>/frontend/src/app/storage/page.tsx
```

docker-compose.yml（ストレージ選択時）:
- PostgreSQL 未選択時: MinIO 用 docker-compose.yml をコピー
```bash
cp ${CLAUDE_PLUGIN_ROOT}/templates/with-storage/docker-compose.yml /Users/user/<プロジェクト名>/docker-compose.yml
```
- PostgreSQL 選択時（docker-compose.yml がコピー済み）: Edit ツールで MinIO サービスを追加する。追加する内容は `${CLAUDE_PLUGIN_ROOT}/templates/with-storage/docker-compose.yml` を参照し、services: に minio と minio-init を追加、volumes: に miniodata: を追加する。

Redis 選択時:
```bash
# Go ソース（registry, handler, infrastructure）を追加
cp -r ${CLAUDE_PLUGIN_ROOT}/templates/with-redis/backend/internal/. /Users/user/<プロジェクト名>/backend/internal/

# フロントエンド /cache ページを追加
mkdir -p /Users/user/<プロジェクト名>/frontend/src/app/cache
cp ${CLAUDE_PLUGIN_ROOT}/templates/with-redis/frontend/src/app/cache/page.tsx /Users/user/<プロジェクト名>/frontend/src/app/cache/page.tsx
```

docker-compose.yml（Redis 選択時）:
- 他のオプション未選択時: Redis 用 docker-compose.yml をコピー
```bash
cp ${CLAUDE_PLUGIN_ROOT}/templates/with-redis/docker-compose.yml /Users/user/<プロジェクト名>/docker-compose.yml
```
- 他のオプション選択済み（docker-compose.yml がコピー済み）: Edit ツールで Redis サービスを追加する。追加する内容は `${CLAUDE_PLUGIN_ROOT}/templates/with-redis/docker-compose.yml` を参照し、services: に redis を追加、volumes: に redisdata: を追加する。
...
# agents/ をコピー（Plugin構造から）
cp ${CLAUDE_PLUGIN_ROOT}/agents/*.md /Users/user/<プロジェクト名>/.claude/agents/

# skills/ をそのままコピー（ディレクトリ構造ごと）
cp -r ${CLAUDE_PLUGIN_ROOT}/skills /Users/user/<プロジェクト名>/.claude/skills

# hooks.json を settings.json に変換してコピー
cp ${CLAUDE_PLUGIN_ROOT}/hooks/hooks.json /Users/user/<プロジェクト名>/.claude/settings.json
```
...
Ghostrunnerの `.claude/CLAUDE.md` (`${CLAUDE_PLUGIN_ROOT}/.claude/CLAUDE.md`) の構造を参考に、新プロジェクト用に生成する。
...
まずPlugin rootのパスを取得する:
```bash
PLUGIN_ROOT=$(echo ${CLAUDE_PLUGIN_ROOT})
# フォールバック: 環境変数が空の場合、既知のパスを検索
if [ -z "$PLUGIN_ROOT" ]; then
  PLUGIN_ROOT=$(find ~/.claude/plugins -name "ghostrunner" -type d 2>/dev/null | head -1)
fi
if [ -z "$PLUGIN_ROOT" ]; then
  PLUGIN_ROOT="/Users/user/Ghostrunner"
fi
```

シンボリックリンクを作成:
```bash
ln -s "$PLUGIN_ROOT/devtools" /Users/user/<プロジェクト名>/.devtools
```
