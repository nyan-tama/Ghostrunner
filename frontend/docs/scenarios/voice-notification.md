...
Ghost Runner には2つの通知経路がある。

| 通知方式 | 実行場所 | 有効条件 | 用途 |
|---------|---------|---------|------|
| 音声通知（OpenAI Realtime API） | フロントエンド | Voice notification トグル ON | 音声での完了/エラー通知、対話モード |
| プッシュ通知（ntfy.sh） | バックエンド | 環境変数 `NTFY_TOPIC` 設定時 | スマートフォン等へのプッシュ通知 |

プッシュ通知（ntfy.sh）はバックエンドが自動で送信するため、フロントエンドでの操作は不要。ntfy.sh クライアントアプリをスマートフォンにインストールし、同じトピックを購読することで通知を受け取れる。

以下のシナリオはフロントエンドの音声通知機能に関するもの。
