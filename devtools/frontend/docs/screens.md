...
| `/patrol` | `app/patrol/page.tsx` | 巡回ダッシュボード（複数プロジェクトの自動巡回・リアルタイム監視） |
...
`/patrol` は複数プロジェクトの自動巡回を管理するダッシュボード。登録したプロジェクトの状態を SSE でリアルタイム監視し、承認待ちの質問に回答できる。
