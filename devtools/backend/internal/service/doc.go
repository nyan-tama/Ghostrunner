...
// # TTSService
//
// VOICEVOXエンジンを使ったテキスト音声合成を担当するサービス。
// NewTTSService() は常に非nilのサービスを返す（VOICEVOXはローカルエンジンのため
// APIキーが不要）。接続先はVOICEVOX_HOST環境変数で設定する。
//
// 主なメソッド:
//   - Synthesize: テキストを音声合成しWAVバイナリを返却
//
// 最適化:
//   - singleflight: 同一テキストへの同時リクエストを1回の合成に集約
//   - LRUキャッシュ: 合成結果をメモリにキャッシュし、同一テキストの再リクエストを高速化
//
// エラー分類:
//   - 接続拒否・タイムアウト: VOICEVOXエンジンが起動していない、または応答がない
//   - 上流エラー: VOICEVOXエンジンが非200ステータスを返した
//   - レートリミット: リクエスト頻度が閾値を超えた
//
// # 画像サポート
//
