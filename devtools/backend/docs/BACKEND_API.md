...
| `ELEVENLABS_API_KEY` | No | ElevenLabs API のAPIキー。設定すると `/api/tts` が ElevenLabs にプロキシし、未設定時は503を返してフロントエンドがWeb Speech APIへフォールバックする |
| `ELEVENLABS_DEFAULT_VOICE_ID` | No | TTSで使用するデフォルトのvoice_id。未指定時は `KgETZ36CCLD1Cob4xpkv`（Romaco）。リクエストでvoice_idが省略された場合に使用される |
| `ELEVENLABS_DEFAULT_MODEL` | No | TTSで使用するデフォルトのmodel_id。未指定時は `eleven_flash_v2_5` |
...
| `/api/tts` | POST | ElevenLabs TTS 音声合成プロキシ（audio/mpegを返却） |
...
| 429 | レート制限・クレジット超過（TTS等の外部APIプロキシで上流が429を返した場合） |
| 500 | Claude CLI実行エラー |
| 502 | 上流APIエラー（TTS等の外部APIプロキシで上流が4xx/5xxを返した場合の集約） |
| 503 | サービス無効（必要な環境変数が未設定の場合） |
| 504 | 上流APIタイムアウト（TTS等の外部APIプロキシで応答が間に合わなかった場合） |
...
## TTS API（ElevenLabs プロキシ）

ElevenLabs Text-to-Speech APIをbackendで中継し、フロントエンドへ音声バイナリ（audio/mpeg）を返却するエンドポイント。APIキーをbackendに隔離した上で、インメモリLRUキャッシュとsingleflightにより同一テキストの重複読み上げを抑制する。

### 概要

- 一括方式（非ストリーミング）でElevenLabs APIを呼び出し、MP3バイナリ（`mp3_44100_128`）をそのままレスポンスとして返す
- `ELEVENLABS_API_KEY` が未設定の場合は503を返し、フロントエンドはWeb Speech APIへフォールバックする
- voice_id / model_id はbackend側の環境変数で固定し、リクエストでは `text` のみを受け付ける
- キャッシュキーは `SHA256(text + voice_id + model_id + output_format)`、TTLは24時間、合計サイズ上限は50MB
- 同一キーへの重複リクエストは singleflight で1本に統合し、上流呼び出しの増幅を防ぐ
- 上流の401（APIキー無効）はbackendログのみで把握し、フロントには502に丸める

### POST /api/tts

テキストを受け取り、ElevenLabsの音声合成結果をMP3バイナリで返却する。

#### 必要な環境変数

| 環境変数 | 説明 |
|----------|------|
| `ELEVENLABS_API_KEY` | ElevenLabs APIキー（必須） |
| `ELEVENLABS_DEFAULT_VOICE_ID` | デフォルトのvoice_id（未指定時 `KgETZ36CCLD1Cob4xpkv`） |
| `ELEVENLABS_DEFAULT_MODEL` | デフォルトのmodel_id（未指定時 `eleven_flash_v2_5`） |
...
| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `text` | string | Yes | 読み上げ対象のテキスト。1〜2000文字 |
...
ボディはMP3バイナリ（`mp3_44100_128`）。

| ヘッダ | 説明 |
|-------|------|
| `Content-Type` | 常に `audio/mpeg` |
| `Content-Length` | レスポンスボディのバイト数 |
| `X-TTS-Cache` | キャッシュヒット時 `hit`、上流呼び出し時 `miss` |
...
| コード | 説明 |
|--------|------|
| 200 | 音声合成成功（audio/mpegを返却） |
| 400 | リクエスト不正（textが空、2000文字超過、JSON不正等） |
| 429 | ElevenLabs側のレート制限・クレジット超過 |
| 502 | 上流APIエラー（ElevenLabs側で4xx/5xxを返した場合に集約。401キー無効もここに丸められる） |
| 503 | `ELEVENLABS_API_KEY` 未設定（TTS機能が無効） |
| 504 | 上流APIタイムアウト（ElevenLabsからの応答が間に合わなかった場合。retryなし） |

#### キャッシュ動作

- キー: `SHA256(text + voice_id + model_id + output_format)` の16進文字列
- TTL: 24時間（書き込み時刻から）
- 合計サイズ上限: 50MB（バイト数を厳密に管理し、LRUで古いエントリを追い出し）
- エントリ数上限: 1<<20（バイト数上限と組み合わせて実質バイト数で制御）
- singleflight: 同一キーへの重複リクエストは1本に統合し、結果はキャッシュに書き込まれるため後続は即hitに倒れる
...
レスポンスヘッダで `X-TTS-Cache` を確認するには `-D -` を付ける:
