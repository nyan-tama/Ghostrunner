...
- **iOS / SwiftUI アプリの場合: フェーズ0.5, フェーズ1, フェーズ2をスキップし、フェーズ I（iOS）を実行**
...
タスクファイルの位置に応じて着手移動を行う:

- タスクが `実装待ち/` にある場合（= 手動実行・引数が `@...実装待ち/...`）:
  ```bash
  mkdir -p "開発/実装/実行中"
  mv "開発/実装/実装待ち/<ファイル名>" "開発/実装/実行中/"
  ```
- タスクが既に `実行中/` にある場合（= gr-run 経由・引数が `@...実行中/...`）:
  何もしない（gr-run が os.Rename で移動済み）。
...
**エスカレーション:** planner でも解決できない問題（仕様の矛盾、技術的に実現不可能等）が発生した場合は、計画書に確認事項を追記して停止する。フォーマットは `.claude/agents/chief-director.md` の正規形（SSOT）に従う:
```
## 確認事項
...
## フェーズ I: iOS / SwiftUI アプリ実装（iOS アプリの場合のみ）

iOS / SwiftUI アプリの場合、フェーズ1・2 の代わりにこのフェーズを実行する。
フロントエンド/バックエンドの2層ではなく、単一アプリとして1サイクルで完了。
各エージェントは自分の責務だけを行い、残りは次の専門エージェントにバトンパスする。

```mermaid
flowchart TD
    A[ios-impl] -->|実装完了| B[ios-reviewer]
...
- **本番コードのバグ**が見つかった場合 → I.1 に戻って impl が修正
- **テストコードの問題**の場合 → tester が自分で修正して再実行
...
タスクの元となった仕様書（plan ファイル）を `実行中/` から `完了/` に移動する。

gr-run 経由時は `実行中/` のファイルが untracked（os.Rename で移動されたため）なので `git mv` は使えない。
手動実行時と gr-run 経由時の両方に対応するため、**plain mv + 当該パス限定の git add** で統一する:

```bash
mkdir -p "開発/実装/完了"
mv "開発/実装/実行中/<仕様書ファイル名>" "開発/実装/完了/"
# スコープ限定: 当該タスクファイルのパスのみステージ（他タスクの混入防止）
git add -- "開発/実装/実装待ち/<仕様書ファイル名>" "開発/実装/実行中/<仕様書ファイル名>" "開発/実装/完了/<仕様書ファイル名>"
```

注: 存在しないパスの `git add` は無視されるため、3パス全て指定して安全。
...
### iOS / SwiftUI アプリ
- iOS Simulator 向けビルド（`xcodebuild`）が warning / error 0 でパス
- レビューで Critical / Warning の指摘がない
