...
主対象が `.github/workflows/` のワークフロー・`scripts/*smoke*` 等のスモーク・`*/e2e/*.spec`（Playwright）など「デプロイ後の本番システムが動くかを検証するハーネス」の場合は `ci-infra-planner` を使う（アプリ機能コードの planner ではない）。
...
**確認事項フォーマット:** 計画書内で判断が必要な事項は `.claude/agents/chief-director.md` の正規形（SSOT）に従って記載する。`**ステータス**: 未回答` / `**ステータス**: 回答済` の太字マーカーを使い、chief-director や gr-run が検出できる形式を維持すること。
...
**CI/インフラ計画では本ステップ（4.5）をスキップする。** 検証ハーネス（ワークフロー・スモーク・E2E）はそれ自体がテストであり、テストプランを別途作るのは入れ子になる。検証の十分性は `ci-infra-plan-reviewer` の anti-false-green 監査（走った証明・意図skip・否定系の理由・break-confirm-red の受け入れ条件化）で担保する。
...
仕様書が `開発/検討中/` にある場合、計画書の作成が完了した時点でアーカイブに移動する:

```bash
git mv "開発/検討中/<元ファイル名>.md" "開発/検討中/アーカイブ/<元ファイル名>.md"
```

`開発/検討中/` 以外（直接引数で渡された場合等）はこのステップをスキップする。
