...
**重要: レポートは別ファイルに作成しない。必ず計画書（`_plan.md`）の末尾に `---` 区切りで追記する。**

- 計画書・仕様書がある場合: そのファイルの末尾に追記（別ファイル作成は禁止）
- 計画書がない場合のみ: 新規ファイルとして作成
...
**別ファイル（`_report.md`）の作成は禁止。** 必ず計画書ファイルの末尾に追記する。

1. 計画書ファイル（`_plan.md`）の末尾に `---` 区切り線を入れてレポートを追記
2. ファイル移動は呼び出し元の指示に従う

**移動する場合（全実装完了時）:**
```bash
mv 開発/実装/実装待ち/機能名.md 開発/実装/完了/機能名.md
```
...
flowchart LR
    subgraph Before["実装前"]
        SPEC_BEFORE["開発/実装/実装待ち/機能名.md"]
    end

    subgraph After["実装後"]
        SPEC_AFTER["開発/実装/完了/機能名.md"]
        REPORT["+ 実装完了レポート"]
    end
